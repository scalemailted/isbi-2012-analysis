...
def test_model(model_fn, train_loader, test_loader, train_steps=50, val_steps=50, 
    ...
    for i in range(iterations):
        ...
        patience = 1
        reduce_lr = ReduceLROnPlateau(monitor='val_accuracy', factor=0.1, patience=patience-1, verbose=1)
        
        # Stop training when validation accuracy decreases on subsequent epoch
        early_stopping = EarlyStopping(monitor='val_accuracy', patience=patience+1, verbose=1)
        callbacks = [reduce_lr, early_stopping]
        
        # Save best model if path provided
        if save_pth:
            save_model = ModelCheckpoint(save_pth, save_best_only=True, monitor='val_accuracy')
...
def hists2df(hists:list):
    ...
    for experiment in hists:
        epoch_count = len(experiment['val_accuracy'])
